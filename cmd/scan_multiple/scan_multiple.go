...
func do(cmd cmdData) error {
	...
	jobChannel := make(chan int, 32*cmd.numWorkers)
	for i := 0; i < cmd.numWorkers; i++ {
		...
		//template["Segment"] = i
		go worker(cmd, template, jobChannel, lineOutChannel, terminationChannel)
	}
...
func worker(cmd cmdData, template map[string]interface{}, jobChannel <-chan int, ch chan<- []byte, termination chan<- error) {
	var response Response
	for job := range jobChannel {
		for {
			template["TableName"] = strconv.Itoa(job)
